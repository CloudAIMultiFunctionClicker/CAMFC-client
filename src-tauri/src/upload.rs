...
impl ChunkUploader {
    ...
    pub async fn init_upload(&self, _filename: &str, _total_size: u64) -> Result<String> {
        ...
        // 解析响应，获取 upload_id
        let response_data: InitUploadResponse =
            response.json().await.context("解析初始化响应失败")?;

        println!(
            "上传初始化成功，获取到 upload_id: {}",
    ...
    pub async fn upload_chunk(
        ...
        let form = multipart::Form::new().part(
            ...
            multipart::Part::bytes(chunk_data.to_vec())
                .file_name(format!("chunk_{:04}", chunk_index)),
        );
    ...
    pub async fn finish_upload(
        ...
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await.unwrap_or_default();
            return Err(anyhow::anyhow!("完成上传失败: {} - {}", status, error_text));
        }
    ...
    pub async fn get_upload_status(&self, upload_id: &str) -> Result<Vec<u32>> {
        ...
        // 解析响应，获取已上传分片列表
        let status_data: UploadStatusResponse =
            response.json().await.context("解析上传状态失败")?;
...
impl UploadTask {
    ...
    pub async fn new(
        ...
        println!(
            "创建上传任务: {}, 大小: {} 字节, 分片数: {}",
            filename, total_size, chunks_total
        );
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.status.lock().await = UploadStatus::Uploading;
        ...
        // 打开文件
        let mut file = File::open(&self.file_path).await.context("打开文件失败")?;
        ...
        for chunk_index in 0..self.chunks_total {
            ...
            for retry_count in 0..3 {
                ...
                match self
                    .uploader
                    .upload_chunk(&self.upload_id, chunk_index, &chunk_data)
                    .await
                {
                    Ok(_) => {
                        // 更新进度
            ...
            if let Some(e) = last_error {
                *self.status.lock().await =
                    UploadStatus::Error(format!("分片 {} 上传失败: {}", chunk_index, e));
        ...
        // 所有分片上传完成，调用完成接口
        eprintln!(
            "[start] 所有分片上传完成，共 {} 个分片，准备调用 finish_upload",
            self.chunks_total
        );
