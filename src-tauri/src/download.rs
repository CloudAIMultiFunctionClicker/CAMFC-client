...
// 导入工具模块
use crate::utils::network::{create_http_client, CHUNK_SIZE};
...
impl ChunkDownloader {
    ...
    pub async fn download_chunk(
        ...
        headers.insert(header::RANGE, header::HeaderValue::from_str(&range_header)?);
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow::anyhow!("下载请求失败: {} - {}", status, error_text));
        }

        // 读取响应内容
        let chunk_data = response.bytes().await.context("读取分片数据失败")?;
...
impl DownloadTask {
    // 创建新的下载任务
    pub async fn new(file_id: String, save_path: PathBuf, auth_info: AuthInfo) -> Result<Self> {
        // 创建下载器
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.status.lock().await = DownloadStatus::Downloading;
        ...
        if self.save_path.exists() {
            let file_size = fs::metadata(&self.save_path)
                .await
                .context("检查已下载文件失败")?
                .len();
        ...
        for chunk_index in starting_chunk..chunks_count {
            ...
            for retry_count in 0..3 {
                ...
                match self
                    .downloader
                    .download_chunk(&self.file_id, chunk_index, start, end)
                    .await
                {
                    Ok(chunk_data) => {
                        // 检查分片大小是否合理
            ...
            if let Some(e) = last_error {
                *self.status.lock().await =
                    DownloadStatus::Error(format!("分片 {} 下载失败: {}", chunk_index, e));
    ...
    async fn write_chunk(&self, offset: u64, data: &[u8]) -> Result<()> {
        ...
        // 验证当前位置是否正确
        let actual_pos = file.stream_position().await.context("获取当前位置失败")?;
        ...
        // 写入数据
        file.write_all(data).await.context("写入文件失败")?;

        // 确保数据写入磁盘
        file.flush().await.context("刷新文件失败")?;

        // 验证写入后的文件大小
        let new_file_size = file
            .metadata()
            .await
        ...
        if new_file_size < expected_new_size {
            println!(
                "警告: 写入后文件大小 {} < 期望大小 {}",
                new_file_size, expected_new_size
            );
        }
    ...
    let download_dir = match custom_path {
        ...
        _ => {
            ...
            if cfg!(target_os = "windows") {
                let user_profile = std::env::var("USERPROFILE").context("获取用户目录失败")?;
...
pub async fn calculate_file_hash(path: &Path) -> Result<String> {
    let mut file = File::open(path).await.context("打开文件失败")?;

    let mut hasher = Sha256::new();
    let mut buffer = vec![0u8; 8192]; // 8KB缓冲区

    loop {
        let bytes_read = file.read(&mut buffer).await.context("读取文件失败")?;
